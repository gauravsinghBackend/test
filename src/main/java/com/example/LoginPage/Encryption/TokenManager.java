...
public class TokenManager {
    ...
    //    private static final String SECRET_KEY_STRING = "00112233445566778899aabbccddeeff"; // Replace with your secret key
    private static final String SECRET_KEY_STRING = "2f6d59ecea7276eb";
    private SecretKeySpec secretKeySpec;

    public TokenManager() {
        secretKeySpec = new SecretKeySpec(SECRET_KEY_STRING.getBytes(StandardCharsets.UTF_8), ALGORITHM);
    }

    public String generateToken(Long password) throws Exception {
        String password1=password.toString();
        Cipher cipher = Cipher.getInstance(TRANSFORMATION);
        cipher.init(Cipher.ENCRYPT_MODE, secretKeySpec);

        byte[] encryptedBytes = cipher.doFinal(password1.getBytes(StandardCharsets.UTF_8));

        return Base64.getEncoder().encodeToString(encryptedBytes);
    }

    public TokenData decryptToken(String encryptedPassword) throws Exception {
        Cipher cipher = Cipher.getInstance(TRANSFORMATION);
        ...
        tokenData.setUserId(Long.parseLong(s));
        return tokenData;
    }
}
